...
```bash
curl -fsSL https://github.com/wso2/agent-manager/releases/download/amp/v1.0.0-rc1/start.sh -o start.sh
chmod +x start.sh
./start.sh
```
...
docker run --rm -it --name amp-quick-start \
  ...
  --network=host \
  ghcr.io/wso2/amp-quick-start:v1.0.0-rc1
```

Either way, the container runs `install.sh` automatically as soon as it starts, then drops you into a shell inside the container once installation completes.
