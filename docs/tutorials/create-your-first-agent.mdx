...
- **Ballerina**: No Start Command or Language Version to set. The platform builds and runs the project directly. See [Deploy the Ballerina Leave Assistant Sample](../guides/build-a-ballerina-agent.mdx) for a ready to deploy example.
- **Docker**: Use a `Dockerfile` in the repository.

**Enable auto instrumentation** *(checked by default)*

For Python agents, this injects OpenTelemetry tracing into your agent with zero code changes, and lets you pin the **AMP Instrumentation Version** used by the init container.

For Ballerina agents, this sets up trace publishing automatically, with no extra configuration needed.
