...
A gateway's type is fixed when it first registers and cannot be changed afterwards, so an environment's topology cannot be switched in place. To move an existing environment to split topology, delete it and create it again.
