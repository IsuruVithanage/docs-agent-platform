...
| Deploy | `amp:agent:env-non-production` | Deploys a built artifact to the lowest [environment](./environment.mdx) in the project's [pipeline](./deployment-pipeline.mdx). Needs `amp:agent:env-production` instead if that environment is flagged as production. |
| Promote | `amp:agent:env-non-production` | Moves a deployed version to the next environment in the pipeline. Needs `amp:agent:env-production` instead when the target is a production environment. |
