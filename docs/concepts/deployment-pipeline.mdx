...
- Deploying, promoting, and suspending each require a scope for the *kind* of environment being targeted: `amp:agent:env-non-production` for ordinary environments, `amp:agent:env-production` for those flagged `isProduction`. So the access-control boundary follows the production flag rather than a position in the pipeline — someone can be allowed to push all the way to staging without being allowed into production, whatever the pipeline's shape.
