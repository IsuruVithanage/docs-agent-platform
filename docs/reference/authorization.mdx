...
| `amp:agent:env-non-production` | Act on a non-production environment — deploy, promote, or change deployment state |
| `amp:agent:env-production` | Act on an environment flagged `isProduction` — the same three operations, in production |
...
Deploying and promoting have no separate capability scope — within an environment
the tier scope is the whole grant, because being able to deploy into an
environment and being able to stop something running there amount to the same
authority over what runs in it.
...
| `deploy_agent` | `amp:agent:env-non-production` |
| `update_deployment_state` | `amp:agent:suspend` + `amp:agent:env-non-production` |
