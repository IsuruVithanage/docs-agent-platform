...
*Upgrading an existing install: the token scopes for MCP clients come from permissions registered on the MCP resource server in Thunder (script `60-amp-resource-server.sh`), which are seeded by the bootstrap job only. On installs seeded before these permissions existed, re-run the bootstrap job (a `helm upgrade` alone will not re-seed) — otherwise MCP tokens are issued with an empty scope list and every tool call returns 403.*
