...
*Upgrading an existing install: the observability scopes in the token come from permissions registered on the observer MCP resource server in Thunder, seeded by the bootstrap job only — re-run the bootstrap job on installs seeded before these permissions existed (a `helm upgrade` alone will not re-seed), otherwise tokens carry an empty scope list and every tool call returns 403.*
