...
**The data-plane store is yours to choose.** Deployment secrets — the environment variables of deployed agents — are pushed and pulled entirely through External Secrets, so this plane can point at a cloud provider's secret manager instead. See [using a cloud secret manager for deployed agents](#using-a-cloud-secret-manager-for-deployed-agents) below. Keeping everything in OpenBao is also a perfectly good answer, and is what the rest of this guide assumes.
...
#### Using a cloud secret manager for deployed agents

Optional. Skip this if OpenBao is holding everything, which is the rest of this guide's assumption.

Deployed agents' environment variables are the one class of secret that never passes through the Vault API: Agent Manager hands them to the OpenChoreo secret API, which pushes them into whatever store the **data plane** names and generates the ExternalSecret that reads them back. Both halves go through External Secrets, so this plane can point at [any provider External Secrets supports](https://external-secrets.io/latest/provider/aws-secrets-manager/) — AWS Secrets Manager, GCP Secret Manager, Azure Key Vault among them.

Add a second store alongside `default`, using your provider's own authentication:
...
`secretStoreRef` is where deployed agents' environment variables are stored. Point it at a cloud provider's secret manager instead if you set one up in [Step 2](#using-a-cloud-secret-manager-for-deployed-agents).
