...
`*.${BASE_DOMAIN}` also covers the per-environment Thunder instances provisioned in Phase 2 — their hostnames (`<handle>.${BASE_DOMAIN}`) sit directly under `BASE_DOMAIN` with no fixed subdomain segment in between, so no separate record is needed for them.
...
There is no `<org>-<env>` pattern to compute the issuer from — `add-environment-thunder.sh` always registers an unguessable handle (printed as `Issuer:` in its own summary output above), so use that value here:
