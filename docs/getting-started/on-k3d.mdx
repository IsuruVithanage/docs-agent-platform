...
:::note No HTTPS ingest on this setup
The `ClusterDataPlane` manifest in [Step 8](#step-8-setup-data-plane) requests both an `http` and `https` external ingress listener, but OpenChoreo 1.1.1 only provisions the `http` one (port 19080) — the Data Plane's `gateway-default` Service has no 19443 listener, so an HTTPS OTLP endpoint is not currently reachable on this single-cluster k3d setup. Use the HTTP endpoint above for trace ingestion until this is fixed upstream in OpenChoreo.
:::
