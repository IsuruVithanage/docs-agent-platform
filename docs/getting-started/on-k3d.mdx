...
:::note HTTPS ingest is disabled by default on this setup
...
To enable it, set `gateway.tls.enabled: true` in `values-dp.yaml` before Step 8's `helm install`, or apply it afterwards with:
```bash
...
```
This isn't OpenChoreo lacking HTTPS support — the port, hostname, and certificate reference are already wired up in the values file; TLS is just switched off. Use the HTTP endpoint above for trace ingestion unless you've enabled and verified this yourself.
:::
