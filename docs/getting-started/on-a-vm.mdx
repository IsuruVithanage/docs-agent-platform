...
flowchart TB
    ...
    caddy -->|"*.amp.&lt;IP&gt;.sslip.io<br/>on-demand TLS"| cpgw
...
| `https://<handle>.<IP>.sslip.io` | Per-environment Thunder (one unguessable host per environment, created on demand) |
...
The same applies to the per-environment Thunder hosts (`<handle>.<IP>.sslip.io`): they are created when you add an environment and get their certificates on demand the same way, so the first request to a brand-new environment's Thunder can show the same one-time error.
...
The two wildcard routes matter because those tiers are **dynamic**: deploying into a new org/project adds a `<org>-<project>.agents.<DOMAIN_BASE>` host (agents in an existing project share theirs), and each environment you create adds a `<handle>.<DOMAIN_BASE>` host (an unguessable handle, sitting directly under the base domain with no fixed subdomain segment in between). `*.agents.<DOMAIN_BASE>` and `*.<DOMAIN_BASE>` are matched by the front-proxy routes and covered by the wildcard certificate up front, so nothing has to be re-issued or re-attached after install.
...
Per-environment Thunder hosts sit directly under `DOMAIN_BASE` (`<handle>.amp.mycompany.com`, no fixed `thunder.` segment in between), so the single top-level wildcard covers them the same way it covers `console`/`api`/`observer`/`gateway`/`cp` — no separate wildcard or explicit A record is needed for them.

Verify the fixed hosts resolve before installing:

```bash
for h in console.amp.mycompany.com thunder.amp.mycompany.com \
         api.amp.mycompany.com x-y.agents.amp.mycompany.com; do
  printf '%-40s %s\n' "$h" "$(dig +short "$h" A)"
done
```
...
`/etc/hosts` has no wildcard support, so the dynamic `*.agents.<DOMAIN_BASE>` and per-environment Thunder hosts are not covered by the lines above. Each time you deploy an agent or create an environment, add its exact hostname:
