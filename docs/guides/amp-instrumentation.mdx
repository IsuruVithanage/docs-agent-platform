...
The zero-code path instruments a fixed set of agent frameworks, shipped inside the Traceloop SDK that each AMP-instrumentation version pins. **The framework set is the same across all AMP-instrumentation versions and all supported Python versions.** The only difference is LiteLLM, which is absent from 0.3.0 and present from 0.4.0 onward. So you don't need to pick a specific version to get a specific framework.

### Verified frameworks

These are exercised on every release by the [instrumentation matrix test suite](https://github.com/wso2/agent-manager/tree/main/test/instrumentation-matrix), which asserts the exact span kinds listed:
