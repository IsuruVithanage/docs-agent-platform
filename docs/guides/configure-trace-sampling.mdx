...
|---|---|---|
| Where | Console → **Deploy** → **Configure & Deploy** → **Environment Variables** | The agent's own environment (shell, Dockerfile, systemd unit, Kubernetes manifest) |
...
2. Click **Configure & Deploy** on the environment you want to sample.
3. Under **Environment Variables**, add `OTEL_TRACES_SAMPLER` and `OTEL_TRACES_SAMPLER_ARG`.
4. Deploy the agent.

You can set the same variables while first creating the agent — the **Configure & Deploy** step offers the same **Environment Variables** section.
